...
**Technical scope.** Built solo in 7 days: React Three Fiber for 3D rendering, 4,270+ GLTF models (KayKit, Kenney, Tiny Treats — all CC0), 139 skeletal animation clips, real OGG audio with synthesized fallbacks, Zustand state management, persistent hex-tile village world with camera-fly zone transitions, and a Mad Libs progression system (5 difficulty levels per quest).
